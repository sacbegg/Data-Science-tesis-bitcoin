...
btc_blockchain <- read.zoo('/Users/sacbe/Documents/Tesis licenciatura/Algoritmos/cleaning/BTC-coinMetricsV2.csv', sep = ",", header =TRUE)
...
```{r}
ggplot(btc_median_cmetric, aes(index(btc_median_cmetric), Close)) +
...
for (i in 1:5) {
  ...
  RMSE[6,i] <- rmse(na.approx(trainc - regresionf.btc))
  ACC[6,i] <- TheilU(trainc, regresionf.btc, na.rm = TRUE)
  svm_list[[i]] <- svmf.btc
  RMSE[7,i] <- rmse(na.approx(trainc - svmf.btc))
  ACC[7,i] <- TheilU(trainc, svmf.btc, na.rm = TRUE)
  length(svmf.btc)
  length(rff.btc)
  rf_list[[i]] <- rff.btc
  RMSE[8,i] <- rmse(na.approx(trainc - rff.btc))
  ACC[8,i] <- TheilU(trainc, rff.btc,na.rm = TRUE)
...
#rff.btc = InvBoxCox(rff.btc, lambda = BoxCox.lambda(testc))
rff.btc = exp(rff.btc)
```
...
ggplot(data=testc, aes(index(testc),Close)) +
  geom_point(data = testc, aes(colour = "Original"))+
...
cat("RMSE: ",rmse(na.approx(testc - rff.btc)))
cat("Theil's U: ",TheilU(testc, rff.btc))
...
ggplot(data=trainc, aes(index(trainc),Close)) +
  geom_point(data = trainc, aes(colour = "Original"))+
  geom_line(data = rff.btc.df, aes(colour = "Predicción"))+
  labs(title = "Random Forest", x = " ", y = " ")+
  theme_minimal()+
  theme(text = element_text(family = "LM Roman 10"))+
...
cat("RMSE: ",rmse(na.approx(trainc - rff.btc)))
cat("Theil's U: ",TheilU(trainc, rff.btc))
```